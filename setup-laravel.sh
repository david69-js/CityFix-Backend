...
check_laravel_exists() {
    if [ -f "/var/www/cityfix/artisan" ] && [ -f "/var/www/cityfix/composer.json" ] && [ -d "/var/www/cityfix/app" ] && [ -d "/var/www/cityfix/config" ]; then
...
create_laravel_project() {
    ...
    su -s /bin/bash www-data -c "
        cp -r /tmp/laravel-temp/* /var/www/cityfix/ 2>/dev/null || true
        cp -r /tmp/laravel-temp/.[!.]* /var/www/cityfix/ 2>/dev/null || true
...
setup_laravel_env() {
    ...
    sed -i "s/DB_HOST=.*/DB_HOST=${DB_HOST:-mysql_cityfix}/g" .env
    sed -i "s/DB_DATABASE=.*/DB_DATABASE=${DB_DATABASE:-cityfix}/g" .env
    sed -i "s/DB_USERNAME=.*/DB_USERNAME=${DB_USERNAME:-cityfix_user}/g" .env
